...
In this song, each verse is built on top of the previous verses in a repeating pattern. Can you use ruby to build up the repeated lyrics programatically?

Note on testing: Unlike previous exercises, this exercise doesn't include an extensive set of test cases. Instead, there is an "acceptance" test to make sure that the final output is correct. A `song.txt` file is included with the exercise, but this is to be used for the acceptance test to validate the solution, not as part of the solution itself.
